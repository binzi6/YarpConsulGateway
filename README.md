...
- [http://localhost:1027](http://localhost:1027) → consul1  
- [http://localhost:1028](http://localhost:1028) → consul2  
- [http://localhost:1029](http://localhost:1029) → consul3  

🧩 可通过 [http://localhost:1027/v1/catalog/services](http://localhost:1027/v1/catalog/services) 查看注册的服务。
...
```bash
curl --request PUT   --data '{"ID":"service-a-1","Name":"service-a","Address":"host.docker.internal","Port":8090,"Check":{"HTTP":"http://host.docker.internal:8090/health","Interval":"10s"}}'   http://localhost:1027/v1/agent/service/register
```

YARP 网关会在 30 秒内自动生成对应路由：
...
更多说明可以查看公账号文章https://mp.weixin.qq.com/s/XTn79zUSD5ST4gNWBiBWng
